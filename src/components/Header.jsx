function Header({ currentPage, onNavigate, cartCount = 0 }) {
    ...
    return (
      <header style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', backgroundColor: '#f8f9fa', marginBottom: '20px' }}>
            ...
            {navItems.map((item) => (
              <li key={item.id}>
                <a
                  ...
                  style={{ textDecoration: 'none', color: currentPage === item.id ? '#007bff' : '#333', fontWeight: currentPage === item.id ? 'bold' : 'normal' }}
                >
                  {item.label}
                </a>
              </li>
